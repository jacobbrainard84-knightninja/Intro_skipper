...
try:
    ...
    class _BooleanOptionalAction(argparse.Action):  # type: ignore[no-redef]
        ...
        def __init__(self, option_strings, dest, default=None, **kwargs):
            ...
            # FIX #7: Strip all kwargs incompatible with nargs=0
...
Config = Dict[str, Any]
# FIX #21: SegmentResult is (start, end, confidence) — kept as-is.
# SegmentDict now includes confidence so callers can filter by it.
SegmentResult = Tuple[float, float, float]
SegmentDict = Dict[str, Tuple[float, float, float]]   # (start, end, conf)
...
class DetectionConstants:
    ...
    # FIX #20: Hash suffix length for truncated keys
...
# FIX #13: Use threading.Event for portable, atomic shutdown signalling.
...
def _sha256_hex(data: bytes) -> str:
    """SHA-256 wrapper compatible with Python 3.8 and 3.9+.

    FIX #11: ``usedforsecurity`` was added in 3.9; fall back gracefully
    so the script runs on Python 3.8 without a TypeError.
    """
    try:
...
def _sha256_incremental() -> "hashlib._Hash":
    """Return an incremental SHA-256 hasher, 3.8-compatible."""
    try:
...
def compute_config_hash(config: Config) -> str:
    ...
    # FIX #11: use the 3.8-compatible helper.
...
def get_skip_key(filepath: str, use_full_path: bool = False) -> str:
    ...
    Algorithm (mirrors Lua ``normalize_key`` exactly):
      1. Take the filename stem (no extension).
      2. Lower-case.
      3. Collapse every common word-separator into a single dot.
      4. Strip characters that are not ASCII alphanumeric or dot.
      5. Remove leading / trailing dots and collapse remaining runs.
      6. Truncate long stems: keep the first (MAX_KEY_LENGTH-8) chars
         and append a 7-char hash fragment so the key is stable and
         unique even when two long filenames share a prefix.
         FIX #20: The old '...' suffix was non-idempotent and was
         stripped on re-normalisation, producing a different key.
    ...
    if len(stem) > max_len:
        # FIX #20: Use a hash suffix so truncation is idempotent and
        # unique.  The suffix itself contains no dots so re-normalising
        # the already-truncated key leaves it unchanged.
        suffix = _sha256_hex(stem.encode())[:CONSTANTS.KEY_HASH_SUFFIX_LEN]
        stem = stem[: max_len - CONSTANTS.KEY_HASH_SUFFIX_LEN - 1] + "-" + suffix

    return stem


def _normalize_filename(name: str) -> str:
    """Legacy alias — normalise a *filename* to a skip-data key."""
...
def escape_lua_string(s: str) -> str:
    ...
    FIX #9: Non-ASCII bytes are now emitted as Lua decimal escapes
    (\\NNN) so that paths containing accented characters or CJK
    codepoints survive in Lua builds that are not UTF-8-aware.
    """
    s = s.replace("\\", "/")
    out: List[str] = []
    for ch in s:
        if ch == '"':
            ...
            # FIX #9: Escape each UTF-8 byte individually so non-ASCII
            # characters are safe in all Lua/VLC builds regardless of the
            # active locale or VLC's internal encoding.
...
def validate_segments(
    ...
    A tolerance of 0.5 s is allowed so that minor floating-point
    over-runs from ffprobe vs. the actual stream length do not
    incorrectly discard legitimate segments.  Any clamping is logged
    at INFO so operators can spot systematic upstream DSP bugs.

    FIX #5: The fraction-of-episode check now uses the clamped end time
    so it cannot produce a fraction > 1.0 for segments that merely
    over-run by the tolerance amount.

    FIX #21: Input and output dicts now carry confidence as the third
    tuple element (start, end, confidence).
    """
    ...
    if "intro" in segments:
        ...
        if s < e <= duration + tolerance:
            # FIX #5: clamp before computing fraction.
    ...
    if "outro" in segments:
        ...
        if e > duration:
            ...
            e = duration
        if s < e <= duration + tolerance:
            # FIX #5: clamp before overlap check.
            e_clamped = min(e, duration)
...
def _max_segment_confidence(segs: SegmentDict) -> float:
    """Return the maximum confidence across all segments in *segs*.

    Used by ``detect_segments_batched`` to choose the best reference
    result when the same episode appears in multiple batches.
...
class FingerprintCache:
    ...
    Thread-safety: a single connection with WAL journal mode and an
    in-process threading.Lock is safe for one process with multiple
    threads.  Running two *separate processes* against the same
    ``cache.db`` simultaneously may produce transient lock contention;
    prefer serialised invocations when automating batch runs.

    FIX #14: All calls to ``_get_conn`` are made while ``_lock`` is
    held.  This is documented explicitly here so future contributors
    do not call ``_get_conn`` from outside a ``with self._lock`` block.
    """
    ...
    def __init__(self, db_path: str = CACHE_DB):
        ...
        # _lock guards both _conn and _pending_writes.
        # INVARIANT: _get_conn() must only be called while _lock is held.
    ...
    def file_hash(self, filepath: str, suffix: str = "",
        ...
        with open(filepath, "rb") as f:
            ...
            # FIX #11: use 3.8-compatible incremental hasher.
    ...
    def store_fingerprint(
        ...
        """Persist a fingerprint, rolling back on any SQLite error.

        FIX #1: The INSERT and the conditional COMMIT are now wrapped in
        a try/except that rolls back on failure so a bad write never
        leaves the connection in an implicit broken transaction.
        """
        try:
            fhash = self.file_hash(filepath, cache_suffix, config_hash)
...
def _run_subprocess_interruptible(
    ...
    Two daemon reader threads continuously drain stdout and stderr so the
    subprocess never blocks on the 64 KB OS pipe buffer.

    FIX #4: The drain threads are now joined inside the ``finally`` block
    unconditionally so they cannot leak after a ``KeyboardInterrupt`` or
    ``TimeoutExpired``.  Previously the join calls were skipped when an
    exception was raised inside the polling loop.
    """
    ...
    try:
        ...
        # FIX #4: Always join drain threads to prevent memory leaks,
        # even when an exception was raised above.
...
def load_raw_audio(path: str) -> np.ndarray:
    """Load a raw f32le audio file, returning an empty array on error.

    FIX #15: Files that exceed MAX_RAW_AUDIO_BYTES now emit a WARNING
...
def reshape_fingerprint(
    ...
    """Reshape a flat or 2-D fingerprint array to (n_frames, n_bands).

    FIX #19: Square matrices (n_bands == n_frames) are ambiguous; a
    debug warning is emitted so callers are aware.
    """
...
def find_common_segments(
    ...
    """Find a consensus segment window across all pairwise similarity matrices.

    FIX #17: Agreement is now normalised by the number of episodes that
    *could* cover each window (i.e. whose similarity matrix is long enough
    to reach that window index), rather than by the global maximum length.
    Previously, short episodes produced low coverage counts for late
    windows but those windows were normalised as if all episodes covered
    them, inflating the apparent agreement fraction.
    """
    if not similarities:
        return None
    ...
    # FIX #17: Track how many matrices actually cover each window.
    ...
    # FIX #17: Normalise by actual coverage so partial-coverage windows
    # are not given artificially high agreement fractions.
...
def find_per_episode_segment(
    ...
    """Locate *ref_segment* inside *target_fp* using cross-correlation.

    FIX #3 (documentation): The FFT cross-correlation returns offsets
    into the *flat* fingerprint array (length = n_frames * n_bands).
    Candidate offsets are snapped to the nearest frame boundary by
    integer-dividing by ``n_bands`` before computing the correlation
    coefficient.  This is correct because each frame occupies exactly
    ``n_bands`` consecutive floats in the flat representation.

    The flat-array cross-correlation approach is kept because computing
    it on the 2-D frame-level array would require a 2-D convolution
    which is significantly more expensive.  The frame-alignment step
    is now explicitly documented to make the intent clear.
    """
    ...
    if t_total >= seg_len and seg_len >= 64:
        ...
        seen: Set[int] = set()
        for idx in top_idx:
            # FIX #3 (documentation): ``idx`` is an offset inside the
            # flat float array.  Dividing by ``nb`` (bands per frame)
            # gives the frame index; multiplying back gives the
            # frame-aligned flat offset ``ai``.  This snaps the
            # cross-correlation peak to the nearest frame boundary.
            fi = int(idx) // nb        # frame-aligned index
            if fi in seen:
                continue
            seen.add(fi)
            ai = fi * nb               # flat offset of frame start
...
def _refine_and_store(
    ...
    """Store per-episode skip segments, refining against the reference.

    FIX #21: All entries written into *results* now carry the confidence
    value as the third element of the tuple so callers can filter by it.
    """
...
def _extract_parallel(
    ...
    """Run fingerprint extraction tasks, isolating per-worker failures."""
...
def detect_segments(
    ...
    if not force:
        ...
        for ep in episodes:
            ...
            if segs:
                # FIX #21: carry confidence through from cache.
                cached[ep["path"]] = {
...
def detect_segments_batched(
    ...
    """Run detection in memory-bounded batches.

    FIX #2: The reference episode's results are now tracked across all
    batches and the highest-confidence result is kept.  Previously the
    first batch's reference result was always used, even if a later batch
    produced a better match due to different companion episodes.  Also, if
    batch 1 produced no result for the reference episode the reference was
    ...
    # FIX #2: Track the best reference result across all batches.
    ...
    for i in range(0, len(remaining), eff):
        ...
        # FIX #2: Keep the highest-confidence result for the reference
        # episode across all batches.
        if ref in br:
        ...
        # Merge non-reference results unconditionally.
...
def _collect_videos_recursive(
    ...
    """Collect video files under *video_dir* up to *max_depth* levels deep.

    FIX #16: Symlinks are skipped to prevent infinite loops.  Results
    are accumulated incrementally rather than materialising the full
    directory tree before returning the first entry.
    """
    out: List[str] = []
    try:
        for item in video_dir.rglob("*"):
            # FIX #16: Skip symlinks to avoid following directory loops.
            if item.is_symlink():
                continue
            # Enforce depth limit relative to the root.
            try:
...
def generate_skip_data(
    ...
    FIX #6: The JSON is now written to a temporary file in the same
    directory and then atomically renamed over the target path.  This
    prevents a partially-written (corrupt) skip_data.json if the process
    is interrupted mid-write.

    FIX #20: Legacy keys that were truncated with the old '...' suffix
    are migrated to the new hash-suffix format so existing skip_data.json
    files remain usable after upgrading.
    """
        ...
        try:
            ...
            if isinstance(existing, dict):
                for old_key, val in existing.items():
                    ...
                    if (norm_key in skip
                            and skip[norm_key] != val
                            and norm_key != old_key):
                        logger.warning(
                            "Key collision during migration: "
                            "'%s' -> '%s' overwrites an existing entry; "
...
-- Do NOT change skip_data.json to include nested objects, arrays,
-- booleans, or null without updating this parser.
...
def _lua_helpers() -> str:
    ...
    # Lua's string library is byte-oriented.  Placing a multibyte UTF-8
    # sequence inside [...] matches each byte independently.  The en-dash
    # (E2 80 93) and em-dash (E2 80 94) are substituted with individual
    # gsub calls BEFORE any bracket class so the E2 lead byte cannot
    # corrupt unrelated characters (e.g. EUR = E2 82 AC).
    return r'''
...
--   2. strip extension
--   3. lower-case
--   4. multibyte UTF-8 dashes -> "." (before bracket classes!)
...
--   9. truncate to MAX_KEY_LENGTH with hash suffix (matches Python)
--
-- FIX #12: Added step 9 so long filenames produce the same key in Lua
-- as in Python's get_skip_key().  Without this, skip data written for
-- a long filename was never found by the Lua lookup.
function normalize_key(name)
    ...
    -- 3. lower-case
    name = name:lower()
    -- 4. multibyte UTF-8 dashes (must precede bracket classes)
    name = name:gsub("\xe2\x80\x93", ".")
    name = name:gsub("\xe2\x80\x94", ".")
    -- 5. single-byte separators
    name = name:gsub("[ \t_%-.,:]+", ".")
    ...
    -- 8. trim leading/trailing dots
    name = name:gsub("^%.+", ""):gsub("%.+$", "")
    -- 9. FIX #12: Truncate long keys with a hash suffix, matching Python.
    local MAX_KEY = 120
    local HASH_LEN = 7
    if #name > MAX_KEY then
        ...
        local h = 5381
        for i = 1, #name do
            h = ((h * 33) + string.byte(name, i)) % (2^32)
        end
        local suffix = string.format("%07x", h):sub(1, HASH_LEN)
        name = name:sub(1, MAX_KEY - HASH_LEN - 1) .. "-" .. suffix
    end
    return name
end
...
IMPORTANT: Manual entries entered via this dialog are SESSION-ONLY.
They are stored under the raw filename and are NOT saved to disk.
They will be lost on Reload or VLC restart.
...
function do_skip_outro()
    ...
    -- FIX #18: Only advance to next item if the current item is NOT
    -- the last one in the playlist, preventing wrap-around.
    ...
    if pok and items and items.children and cok and cur then
        local is_last = false
        if #items.children > 0 then
            local last = items.children[#items.children]
            if last and last.id == cur.id then
                is_last = true
            end
        end
        if not is_last and #items.children > 1 then
...
function safe_skip_outro(input, d, item)
    -- FIX #18: Do not advance past the last playlist item.
    local advanced=false
    ...
    if pok and pl then
        ...
        local cok,cur=pcall(function()
            return pl.current and pl:current() end)
        if iok and items and items.children and cok and cur then
            local is_last = false
            if #items.children > 0 then
                local last = items.children[#items.children]
                if last and last.id == cur.id then is_last = true end
            end
            if not is_last and #items.children > 1 then
                advanced=pcall(function() pl:next() end)
            end
...
# FIX #23: Declare the public API surface explicitly.
__all__ = [
    "detect_segments",
