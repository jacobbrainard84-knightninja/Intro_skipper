...
def get_skip_key(filepath: str, use_full_path: bool = False) -> str:
    ...
    Algorithm (mirrors Lua ``normalize_key`` exactly):
      ...
      6. Truncate long stems to ``CONSTANTS.MAX_KEY_LENGTH`` chars.
...
def _run_subprocess_interruptible(
    ...
    subprocess never blocks on the 64 KB OS pipe buffer.  Without this,
    a verbose ffmpeg run producing > 64 KB of output deadlocks: ffmpeg
    blocks trying to write while Python blocks waiting for it to exit.
    """
...
def _extract_parallel(
    ...
    Each future result is retrieved inside a ``try/except`` so that a
    single corrupted episode or transient OS error does not abort the
    entire batch.  Failed tasks are logged and skipped; surviving
        ...
        with ThreadPoolExecutor(max_workers=nw) as pool:
            ...
            for f in as_completed(futs):
                ...
                try:
                    ...
                    # Isolate the failure: log it and continue with
                    # remaining episodes rather than aborting the batch.
                    logger.error(
...
def generate_skip_data(
    ...
    Keys are normalized stems produced by ``get_skip_key`` so that
    Python (write) and Lua (read) always agree.

    Legacy raw keys present in older files are migrated to normalized
    form.  When a legacy key normalizes to a string that already exists
    in the dictionary (e.g. two slightly different raw filenames for
    the same episode), a warning is emitted so the operator can
    investigate rather than silently losing one entry.

    JSON format note: the file stores a flat object where each value is
    also a flat object with numeric leaf values only.  The Lua JSON
    parser embedded in the VLC extension only handles this structure;
    do not add nested objects, arrays, or booleans without updating the
    parser.
    """
...
def _lua_helpers() -> str:
    ...
    # corrupt unrelated characters (e.g. € = E2 82 AC).
    return r'''
...
def install_vlc_extension(silent: bool = False) -> str:
    ...
    # NO mwait/msleep here: a synchronous sleep on the GUI thread
    # freezes the entire player window for its full duration.
    #
    # Manual timestamp entries (Apply button) are stored in the session
    # table under the raw filename key.  They are intentionally NOT
    # written back to skip_data.json and will not survive a Reload or
    # VLC restart.  This is by design — use --import-timestamps for
    # permanent per-episode overrides.
    # ------------------------------------------------------------------
    lua_ext = f'''--[[
...
-- No sleep — extension runs on the VLC GUI thread.
function seek_to(s)
...
function open_dialog()
    ...
    -- Manual entries are SESSION-ONLY and keyed by raw filename.
    -- They are NOT written to skip_data.json.
    dialog:add_label(
...
'''
    ...
    # ------------------------------------------------------------------
    # Background interface script — isolated event loop, never touches
    # the GUI thread.  mwait IS safe here.
    # ------------------------------------------------------------------
    intf = f'''--[[
...
Note: this script polls skip_data.json every ~60 ticks and auto-skips
intros/outros.  Manual entries made in the extension dialog are NOT
visible here because they are session-only and stored under raw
filename keys in the extension's in-memory table.
]]
local SKIP_DATA_FILE="{escaped}"
local skip_data={{}}
local intro_skipped=false
