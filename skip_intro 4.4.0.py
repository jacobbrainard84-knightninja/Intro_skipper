...
Notes:
    - Requires ffmpeg and ffprobe on PATH (or bundled alongside this script).
    - Automatic detection requires >= 2 episodes.  For a single file use
      --import-timestamps to supply manual timestamps.
    - Run only one process at a time against the same cache.db; concurrent
      processes share a SQLite WAL file and may see lock contention.
    - VLC manual entries (entered via the dialog) are session-only and keyed
      by raw filename; they are NOT written back to skip_data.json.
    - The Lua JSON parser only handles flat objects with numeric leaf values.
      Do not change the skip_data.json structure without updating the parser.
"""
...
Config = Dict[str, Any]
SegmentResult = Tuple[float, float, float]          # (start, end, confidence)
SegmentDict = Dict[str, Tuple[float, float, float]] # (start, end, confidence)
...
# threading.Event gives atomic test-and-set across all Python
# implementations without relying on the GIL.
_shutdown_event = threading.Event()
_active_processes: List[subprocess.Popen] = []
_process_lock = threading.Lock()

# Tracks episodes skipped because every extracted audio chunk was below
# MIN_AUDIO_RMS.  Surfaced to the user in the final summary so "no
# segments detected" is not the only feedback for silent-audio shows.
_silent_episode_warnings: List[str] = []
...
def _sha256_hex(data: bytes) -> str:
    """SHA-256 compatible with Python 3.8 (no usedforsecurity kwarg)."""
    try:
...
def _sha256_incremental() -> "hashlib._Hash":
    """Incremental SHA-256 hasher, Python 3.8-compatible."""
    try:
...
def compute_config_hash(config: Config) -> str:
    ...
    return _sha256_hex(config_str.encode())[:16]


# ------------------------------------------------------------------
# Key normalisation
# ------------------------------------------------------------------
# The algorithm implemented here in Python MUST stay byte-for-byte
# identical to the Lua normalize_key() function below.  Any change to
# either side requires a matching change on the other side.
#
# Canonical steps (numbered so cross-referencing is easy):
#   1.  Take the filename stem (callers strip the extension first).
#   2.  Lower-case.
#   3.  Replace en-dash (U+2013) and em-dash (U+2014) with ".".
#       These are handled before the bracket-class regex so their
#       multi-byte UTF-8 lead byte (0xE2) cannot accidentally match
#       unrelated characters inside [...].
#   4.  Collapse every ASCII separator ( \t _ . - : ) into a single ".".
#       The regex uses a character class with "-" as the LAST character
#       so it is treated literally, not as a range operator.
#   5.  Strip anything that is not [a-z0-9.].
#   6.  Collapse runs of two or more dots into one dot.
#   7.  Strip leading and trailing dots.
#   8.  If longer than MAX_KEY_LENGTH, keep the first
#       (MAX_KEY_LENGTH - KEY_HASH_SUFFIX_LEN - 1) characters and
#       append "-" + the first KEY_HASH_SUFFIX_LEN hex digits of the
#       SHA-256 of the *full* normalised stem.  Using SHA-256 (not
#       djb2) on both sides guarantees identical suffixes.
# ------------------------------------------------------------------

# Compiled once at import time for speed.
_RE_SEPARATOR = re.compile(r"[ \t_.:-]+")   # "-" is LAST → literal
_RE_NON_ALNUM = re.compile(r"[^a-z0-9.]+")
_RE_MULTI_DOT = re.compile(r"\.{2,}")


def _normalise_stem(stem: str) -> str:
    """Apply steps 2-7 of the canonical key algorithm to *stem*.

    The caller is responsible for step 1 (stripping the extension) and
    step 8 (truncation), so this function can be reused by both
    get_skip_key() and the migration path in generate_skip_data().
    """
    s = stem.lower()
    # Step 3: multi-byte dashes before any bracket-class regex.
    s = s.replace("\u2013", ".").replace("\u2014", ".")
    # Step 4: ASCII separators → single dot.
    s = _RE_SEPARATOR.sub(".", s)
    # Step 5: strip non-alphanumeric / non-dot.
    s = _RE_NON_ALNUM.sub("", s)
    # Step 6: collapse repeated dots.
    s = _RE_MULTI_DOT.sub(".", s)
    # Step 7: strip leading/trailing dots.
    s = s.strip(".")
    return s


def _truncate_key(stem: str) -> str:
    """Apply step 8: truncate *stem* if it exceeds MAX_KEY_LENGTH.

    The hash suffix is derived from the *full* normalised stem so the
    result is stable regardless of where truncation happens.  Using
    SHA-256 here (and in the Lua counterpart) guarantees both sides
    produce the same 7-character hex suffix for the same input.
    """
    max_len = CONSTANTS.MAX_KEY_LENGTH
    if len(stem) <= max_len:
        return stem
    hlen = CONSTANTS.KEY_HASH_SUFFIX_LEN
    suffix = _sha256_hex(stem.encode())[:hlen]
    return stem[: max_len - hlen - 1] + "-" + suffix
...
def get_skip_key(filepath: str, use_full_path: bool = False) -> str:
    ...
    unchanged so multiple shows with identical episode filenames can
    coexist in the same JSON file.
    """
    if use_full_path:
        return str(Path(filepath).resolve())
    stem = Path(filepath).stem          # strip extension (step 1)
    return _truncate_key(_normalise_stem(stem))
...
def escape_lua_string(s: str) -> str:
    """Escape *s* for embedding inside a Lua double-quoted string literal.

    Non-ASCII bytes are emitted as individual Lua decimal escapes so
    paths containing accented or CJK characters survive in all VLC
    builds regardless of the active system locale.
    """
    s = s.replace("\\", "/")
    out: List[str] = []
    for ch in s:
        if ch == '"':
            ...
            # Escape each UTF-8 byte individually so non-ASCII is safe
            # in all Lua/VLC builds regardless of locale.
...
def validate_segments(
    ...
    """Clamp and validate segment timestamps against episode *duration*."""
    validated: SegmentDict = {}
    tolerance = 0.5

    if "intro" in segments:
        s, e, conf = segments["intro"]
        s = max(0.0, s)
        if e > duration:
            logger.info(
                "Intro end %.3fs clamped to episode duration %.3fs",
                e, duration)
            e = duration
        if s < e <= duration + tolerance:
            e_clamped = min(e, duration)
            frac = (e_clamped - s) / duration if duration > 0 else 0.0
            if frac > CONSTANTS.MAX_INTRO_FRACTION:
                logger.warning(
                    "Intro is %.0f%% of episode (%s-%s / %s)",
                    frac * 100, format_time(s), format_time(e_clamped),
                    format_time(duration))
            validated["intro"] = (
                _snap_timestamp(s), _snap_timestamp(e_clamped), conf)
        else:
    ...
    if "outro" in segments:
        s, e, conf = segments["outro"]
        s = max(0.0, s)
        if e > duration:
            logger.info(
                "Outro end %.3fs clamped to episode duration %.3fs",
                e, duration)
            e = duration
        if s < e <= duration + tolerance:
            e_clamped = min(e, duration)
...
def validate_episode_set(video_paths: List[str]) -> None:
    ...
    for name in basenames:
        ...
        if tag:
            seasons.add(tag[0])
    if len(seasons) > 1:
        logger.warning("Multiple seasons detected: %s", sorted(seasons))
...
def _max_segment_confidence(segs: SegmentDict) -> float:
    if not segs:
        return -1.0
...
class FingerprintCache:
    """SQLite-backed store for audio fingerprints and skip segments.

    INVARIANT: _get_conn() must only be called while self._lock is held.
    """
    ...
    def store_fingerprint(
        ...
        with self._lock:
            ...
            try:
                ...
                logger.error(
                    "Failed to store fingerprint for %s: %s", filepath, exc)
                try:
                    conn.rollback()
                except Exception:
                    pass
    ...
    def batch_commit(self):
        with self._lock:
            try:
                ...
                logger.error("batch_commit failed: %s", exc)
    ...
    def store_skip_segment(
        ...
        with self._lock:
            ...
            try:
                ...
                logger.error(
                    "Failed to store segment %s/%s: %s",
                    filepath, segment_type, exc)
                try:
                    conn.rollback()
                except Exception:
                    pass
...
def _run_subprocess_interruptible(
    ...
    Drain threads are always joined inside ``finally`` to prevent leaks
    on ``KeyboardInterrupt`` or ``TimeoutExpired``.
    """
        ...
        while proc.poll() is None:
            if _shutdown_requested():
                ...
                try:
                    ...
                    proc.kill()
                    proc.wait()
                raised_exc = KeyboardInterrupt("Shutdown requested")
                break
            if time.monotonic() > deadline:
                proc.kill()
                proc.wait()
                raised_exc = subprocess.TimeoutExpired(cmd, timeout)
                break
    ...
    if raised_exc is not None:
        raise raised_exc  # type: ignore[misc]

    out_bytes = b"".join(stdout_chunks)
    err_bytes = b"".join(stderr_chunks)
    return subprocess.CompletedProcess(
        args=cmd, returncode=returncode or 0,
...
def load_raw_audio(path: str) -> np.ndarray:
    try:
        ...
        if file_size > CONSTANTS.MAX_RAW_AUDIO_BYTES:
            logger.warning(
                "Audio file %s exceeds %.0f MB limit (%.0f MB) - skipping",
                path,
                CONSTANTS.MAX_RAW_AUDIO_BYTES / (1024 * 1024),
                file_size / (1024 * 1024))
...
def reshape_fingerprint(
    ...
    if fp.ndim == 2:
        ...
        if fp.shape[1] == n_bands:
            if fp.shape[0] == n_bands and n_bands == n_frames:
                logger.debug(
                    "reshape_fingerprint: square matrix (%d, %d) — "
                    "assuming (n_frames, n_bands) layout",
                    n_bands, n_frames)
            return fp
...
def compare_fingerprints(
    ...
    if (fp1.size == 0 or fp2.size == 0 or fp1.ndim != 2
        ...
        if fp1.ndim == 2 and fp1.shape[0] < 2:
            logger.debug(
                "Fingerprint too short for reliable correlation "
                "(frames: %d)", fp1.shape[0])
    ...
    valid1 = (s1.flatten() > eps)
    valid2 = (s2.flatten() > eps)
...
def detect_segments_graph(
    ...
    if est_mb > CONSTANTS.GRAPH_MAX_MATRIX_MB:
        logger.info(
            "Graph consensus disabled for %s: estimated matrix %.1f MB "
            "exceeds limit of %d MB - falling back to linear matching",
            segment_type, est_mb, CONSTANTS.GRAPH_MAX_MATRIX_MB)
        return None, None, {}
...
def find_common_segments(
    ...
    actual_coverage = np.zeros(n_windows, dtype=np.float32)

    for _, sm in similarities:
        reach = sm.shape[0]
        actual_coverage[:reach] += 1.0
    ...
    safe_cov = np.maximum(actual_coverage, 1.0)
    ...
    if not np.any(agreeing):
        ...
        logger.warning(
            "No %s segments met min_agree=%d; retrying with %d",
            segment_type, min_agree, fallback_agree)
        agreeing = norm_agr >= fb
        if not np.any(agreeing):
            logger.info(
                "No %s candidate windows reached even the fallback "
                "agreement threshold - check similarity_threshold and "
                "min_segment_duration for this show type",
                segment_type)
            return None
...
def extract_fingerprint_for_region(
    ...
    """Extract and return the energy fingerprint for one audio region.

    Returns None if extraction fails OR if the audio is below
    MIN_AUDIO_RMS (near-silent).  In the silent case the episode
    basename is appended to _silent_episode_warnings so the main
    routine can surface it to the user rather than emitting only a
    generic "no segments detected" message.
    """
    if _shutdown_requested():
        return None
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="intro_skipper_") as tmp:
            ...
            if rms < CONSTANTS.MIN_AUDIO_RMS:
                # Warn visibly rather than silently discarding.
                msg = (
                    f"Near-silent audio (RMS={rms:.2e}) for "
                    f"{os.path.basename(video_path)} at "
                    f"{format_time(start_time)}-"
                    f"{format_time(start_time + duration)} — "
                    f"fingerprint skipped"
                )
                logger.warning(msg)
                with _silent_lock:
                    name = os.path.basename(video_path)
                    if name not in _silent_episode_warnings:
                        _silent_episode_warnings.append(name)
                return None
            nb = config.get("n_bands", 8)
            ...
            if fp.shape[0] < 2:
                logger.debug(
                    "Very short fingerprint (%d frame(s)) for %s - "
                    "detection may be unreliable",
                    fp.shape[0], os.path.basename(video_path))
...
def _refine_and_store(
    ...
    rc = segment[2]
    results.setdefault(ref_path, {})[seg_type] = (rs, re_, rc)
    ...
    for op in [p for p in all_fps if p != ref_path]:
        ...
        if per_ep:
            ...
            results.setdefault(op, {})[seg_type] = (es, ee, per_ep[2])
            ...
            gc_ = gi[2] * 0.7
            logger.info(
                "  %s: %s %s-%s using graph-fallback (conf %.2f)",
                os.path.basename(op), seg_type,
                format_time(gs), format_time(ge), gc_)
            results.setdefault(op, {})[seg_type] = (gs, ge, gc_)
            cache.store_skip_segment(op, seg_type, gs, ge, gc_,
                                     "graph-fallback")
        elif seg_type == "intro":
            logger.info(
                "  %s: intro using fingerprint-fallback "
                "(copied from reference)",
                os.path.basename(op))
            fb_conf = rc * 0.5
            results.setdefault(op, {})[seg_type] = (rs, re_, fb_conf)
            cache.store_skip_segment(op, seg_type, rs, re_,
                                     fb_conf, "fingerprint-fallback")
        elif seg_type == "outro":
            ...
            if ep_dur is not None and ref_dur is not None:
                ...
                if 0 <= fb_s < fb_e <= ep_dur:
                    logger.info(
                        "  %s: outro using fingerprint-fallback "
                        "(end-anchored from reference)",
                        os.path.basename(op))
                    fb_conf = rc * 0.5
                    results.setdefault(op, {})[seg_type] = (
                        fb_s, fb_e, fb_conf)
    ...
    if parallel and len(tasks) > 1:
        ...
        with ThreadPoolExecutor(max_workers=nw) as pool:
            ...
            for f in as_completed(futs):
                ...
                try:
                    ...
                    logger.error(
                        "Worker extraction failed (task %d/%d): %s",
                        done, len(tasks), exc)
    else:
        prog = ProgressTracker(len(tasks), f"{label} fingerprints")
        for idx, task in enumerate(tasks, 1):
            ...
            try:
                ...
                logger.error(
                    "Extraction failed for task %d/%d: %s",
                    idx, len(tasks), exc)
    return out
...
def detect_segments(
    ...
    if not force:
        ...
        for ep in episodes:
            ...
            if segs:
                cached[ep["path"]] = {
                    st: (s, e, c) for st, (s, e, c) in segs.items()}
            else:
                all_cached = False
...
def detect_segments_batched(
    ...
    logger.info(
        "Entering batched detection mode (batch size: %d, "
        "total episodes: %d) to conserve memory",
    ...
    for i in range(0, len(remaining), eff):
        ...
        br, bd = detect_segments(
            ...
            ffmpeg_timeout=ffmpeg_timeout)
        if ref in br:
            conf = _max_segment_confidence(br[ref])
            if conf > best_ref_conf:
                best_ref_conf = conf
                all_res[ref] = br[ref]
    ...
    if "intro_end" in data or "intro_duration" in data:
        ...
        for video in videos:
            ...
            if i_e > 0 and _validate_ts(i_s, i_e, "intro", video.name):
                results[vp]["intro"] = (
                    _snap_timestamp(i_s), _snap_timestamp(i_e), 1.0)
                cache.store_skip_segment(
    ...
    try:
        for item in video_dir.rglob("*"):
            if item.is_symlink():
                continue
            try:
                rel = item.relative_to(video_dir)
...
def generate_skip_data(
    ...
    Migration collision detection covers two distinct cases:

    Case A — a legacy key and its normalised form differ AND the
    normalised form already exists with different data.  This is the
    situation the previous version detected.

    Case B — two *different* original keys both normalise to the same
    string (e.g. "Show S01E01.mkv" and "Show.S01E01.mkv").  The
    previous version missed this because it only checked
    ``norm_key != old_key``.  Now both cases log a WARNING with the
    conflicting keys so the operator can investigate.
    """
    skip: Dict[str, Dict[str, float]] = {}
    # Map from normalised key -> original key(s) that produced it,
    # used to detect case-B collisions during migration.
    norm_origin: Dict[str, str] = {}
        ...
        try:
            ...
            if isinstance(existing, dict):
                for old_key, val in existing.items():
                    ...
                    if norm_key in skip:
                        # Collision: two old keys normalise to the same
                        # string.  Warn regardless of whether the keys
                        # changed (covers both case A and case B).
                        prior_origin = norm_origin.get(norm_key, norm_key)
                        if skip[norm_key] != val:
                            logger.warning(
                                "Key collision during migration: "
                                "'%s' and '%s' both normalise to '%s' "
                                "with different data — keeping the first; "
                                "check for duplicate episode filenames",
                                prior_origin, old_key, norm_key)
                    else:
                        skip[norm_key] = val
                        norm_origin[norm_key] = old_key
    ...
    tmp_fd, tmp_path = tempfile.mkstemp(
        dir=out_dir, suffix=".tmp", prefix="skip_data_")
    try:
        with os.fdopen(tmp_fd, "w", encoding="utf-8") as tmp_f:
            json.dump(skip, tmp_f, indent=2, ensure_ascii=False)
...
def generate_vlc_conf(
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        for fpath, segs in results.items():
            ...
            ie = segs.get("intro", (0.0, 0.0, 0.0))[1]
            od = 0
            if "outro" in segs:
...
def _lua_json_parser() -> str:
    return r'''
-- Minimal JSON parser for skip_data.json.
-- Supports only flat objects whose leaf values are JSON numbers.
...
def _lua_helpers(seek_delay_us: int) -> str:
    """Return the Lua helper library.

    The seek delay is injected here so both the extension dialog and the
    interface script use the same value without duplicating the constant.

    Key design decisions documented inline:
    - normalize_key uses SHA-256 (via a pure-Lua implementation) for the
      truncation suffix, matching Python exactly.
    - seek_with_settle is defined once and used by both seek_to (dialog)
      and seek_and_play (interface), eliminating the stutter that
      previously occurred in the dialog because it had no post-seek delay.
    - safe_advance_playlist checks the current item's *index* in the
      children list rather than comparing .id fields, which are not
      reliably set in all VLC versions.
    """
    return f'''
-- ---------------------------------------------------------------
-- Shared helpers (injected into both the extension and the intf)
-- ---------------------------------------------------------------

local SEEK_SETTLE_US = {seek_delay_us}
...
-- ---------------------------------------------------------------
-- Pure-Lua SHA-256 (RFC 6234 compliant).
-- Used only for the normalize_key truncation suffix so it must
-- produce the same 7 hex characters as Python's hashlib.sha256.
-- ---------------------------------------------------------------
local _sha256_k = {{
...
local function _rotr(x,n) return _u32((x>>n)|(x<<(32-n))) end
local function _sha256_bytes(msg)
    local len = #msg
    local bits = len * 8
    msg = msg .. "\\x80"
    ...
    for blk = 0, (#msg//64)-1 do
        ...
        for i = 16, 63 do
            local s0=_u32(_rotr(w[i-15],7)~_rotr(w[i-15],18)~(w[i-15]>>3))
            local s1=_u32(_rotr(w[i-2],17)~_rotr(w[i-2],19)~(w[i-2]>>10))
...
-- ---------------------------------------------------------------
-- normalize_key — must produce byte-identical output to Python's
-- get_skip_key() for every possible filename.
--
-- Steps:
--   1. percent-decode URI escapes
--   2. strip the last .ext
--   3. lower-case
--   4. en-dash / em-dash (E2 80 93 / E2 80 94) -> "."
--      (done as explicit byte sequences BEFORE any bracket class
--       so the 0xE2 lead byte cannot accidentally match inside [...])
--   5. ASCII separators [ \\t_.-:] -> "."  ("-" is LAST in the class
--      so it is a literal hyphen, not a range operator)
--   6. strip bytes that are not [a-z0-9.]
--   7. collapse ".." -> "."
--   8. strip leading/trailing dots
--   9. if longer than MAX_KEY, truncate and append "-" + first 7
--      hex chars of SHA-256(full_normalised_stem)
-- ---------------------------------------------------------------
local MAX_KEY  = {CONSTANTS.MAX_KEY_LENGTH}
local HLEN     = {CONSTANTS.KEY_HASH_SUFFIX_LEN}
...
function normalize_key(name)
    ...
    -- 3. lower-case
    name = name:lower()
    -- 4. multi-byte dashes (must precede any bracket-class pattern)
    name = name:gsub("\\xe2\\x80\\x93", ".")   -- en-dash U+2013
    name = name:gsub("\\xe2\\x80\\x94", ".")   -- em-dash U+2014
    -- 5. ASCII separators; "-" is last in the class so it is literal
    name = name:gsub("[ \\t_%.%-%:]+", ".")
    -- 6. strip non-alphanumeric / non-dot
    name = name:gsub("[^a-z0-9%.]+", "")
    -- 7. collapse repeated dots
    name = name:gsub("%.%.+", ".")
    -- 8. trim leading/trailing dots
    name = name:gsub("^%.+", ""):gsub("%.+$", "")
    -- 9. truncate with SHA-256 suffix (same algorithm as Python)
    if #name > MAX_KEY then
        local full_hash = _sha256_bytes(name)
        local suffix = full_hash:sub(1, HLEN)
        name = name:sub(1, MAX_KEY - HLEN - 1) .. "-" .. suffix
    end
    return name
end

-- ---------------------------------------------------------------
-- find_skip_data — look up a filename in skip_data (already loaded).
-- Tries normalised key first, then raw key, then full scan.
-- ---------------------------------------------------------------
...
-- ---------------------------------------------------------------
-- seek_with_settle — set the playback position and wait for the
-- decoder to catch up before returning.
--
-- This is used by BOTH the extension dialog (seek_to) and the
-- interface script (seek_and_play).  Previously the dialog's
-- seek_to() had no post-seek delay, causing visible stutter because
-- the audio/video decoders had not flushed their buffers before the
-- next frame was rendered.
--
-- mwait is only available in the interface (intf) context; the
-- extension dialog runs on the VLC GUI thread where mwait must not
-- be called.  The caller passes use_mwait=true only from intf code.
-- ---------------------------------------------------------------
function seek_with_settle(target_s, input, use_mwait)
    ...
    if ok then
        -- Resume playback if the seek left the stream paused.
        pcall(function()
...
-- ---------------------------------------------------------------
-- safe_advance_playlist — advance to the next playlist item only
-- if the current item is not the last one.
--
-- Uses index-based position checking instead of .id comparison
-- because .id is not reliably populated in all VLC versions.
-- Returns true if the playlist was actually advanced.
-- ---------------------------------------------------------------
function safe_advance_playlist()
    local pok, pl = pcall(function() return vlc.playlist end)
    if not pok or not pl then return false end
    ...
    -- Find the index of the currently-playing item by URI comparison.
    ...
    local cur_idx = nil
    for i, child in ipairs(items.children) do
        local cok, child_uri = pcall(function() return child:uri() end)
        if cok and child_uri == cur_uri then
            cur_idx = i
            break
        end
    end

    if cur_idx == nil or cur_idx >= n then
        -- Unknown position or already at last item — do not advance.
        return false
    end

    return pcall(function() pl:next() end)
end
'''
...
def install_vlc_extension(silent: bool = False) -> str:
    ...
    helpers = _lua_helpers(delay_us)

    # ------------------------------------------------------------------
    # Extension dialog
    # Runs on the VLC GUI thread; seek_with_settle is called with
    # use_mwait=false so it never blocks the GUI thread.
    # ------------------------------------------------------------------
    lua_ext = f'''--[[
Intro/Outro Skipper for VLC  (extension dialog)
Version {__version__}

Manual entries entered via this dialog are SESSION-ONLY.
They are stored under the raw filename and are NOT saved to disk.
For permanent per-episode timestamps use --import-timestamps.
]]
function descriptor()
...
-- Extension runs on the GUI thread; use_mwait=false.
function seek_to(s)
    local input=vlc.object.input(); if not input then return false end
    return seek_with_settle(s, input, false)
end
...
function do_skip_outro()
    ...
    -- Try to advance the playlist safely (will not wrap on last item).
    if safe_advance_playlist() then
        outro_skipped=true; return true,"Next"
    end
    -- Fall back to seeking to outro_end or total duration.
    if d.outro_end then
    ...
    if item then
        ...
        if dur and dur>0 then
            if seek_to(dur/1000000) then outro_skipped=true; return true,"End" end
        end
    end
    return false,"Failed"
end
...
function open_dialog()
    ...
    dialog:add_label(
        "<center><b>Manual Override (Session Only)</b></center>",
        1,8,2,1)
    ...
    dialog:add_button("Apply (session only)",function()
        ...
        if iv or ov then
            ...
            if status_label then
                status_label:set_text("Applied (session only)")
            end
            update_dialog_info()
        else
            if status_label then status_label:set_text("Enter numbers") end
        end
    end,1,11,2,1)
...
'''
    ...
    # Runs in its own event loop; mwait IS available here so
    # seek_with_settle is called with use_mwait=true.
    # ------------------------------------------------------------------
    intf = f'''--[[
...
-- Interface script: use_mwait=true so the seek delay is enforced.
function do_seek(target_s, input)
    return seek_with_settle(target_s, input, true)
end
function do_skip_outro_intf(input, d, item)
    -- Try safe playlist advance (no wrap-around on last item).
    if safe_advance_playlist() then return end
    -- Fall back: seek to outro_end or total duration.
    if d.outro_end then
        do_seek(d.outro_end, input)
    else
        local dok,dur=pcall(function() return item:duration() end)
        if dok and dur and dur>0 then
            do_seek(dur/1000000, input)
        end
    end
end
load_data()
                                ...
                                if tok and tus then
                                    ...
                                    if not intro_skipped and d.intro_end then
                                        ...
                                        if t>=is and t<grace_end then
                                            if do_seek(d.intro_end,input) then
                                                intro_skipped=true
                                            end
                                    ...
                                    if not outro_skipped and d.outro_start then
                                        if t>=d.outro_start then
                                            outro_skipped=true
                                            do_skip_outro_intf(input,d,item)
                                        end
                                    end
                                end
                            end
...
def _print_effective_config(config: Config, args: argparse.Namespace):
    print(f"\n{'='*60}")
    print(f"Effective configuration  (show-type: {args.show_type})")
    print(f"{'='*60}")
...
__all__ = [
    "detect_segments",
    "detect_segments_batched",
    "FingerprintCache",
    "generate_skip_data",
    "get_skip_key",
    "SHOW_CONFIGS",
    "install_vlc_extension",
    "import_timestamps_file",
...
def main():
    ...
    if not validate_ffmpeg(FFMPEG):
        logger.error(
            "ffmpeg not found at '%s'.  Install ffmpeg and ensure it is "
            "on PATH, or place a bundled copy alongside this script in an "
            "'ffmpeg-custom/' subdirectory.", FFMPEG)
        sys.exit(1)
    ...
    if args.show_config:
        _print_effective_config(config, args)
        return
    ...
    try:
        with FingerprintCache() as cache:
            try:
                ...
                logger.error("Error reading video directory: %s", exc)
                sys.exit(1)
            if not videos:
            ...
            if args.import_timestamps:
                ...
                logger.error(
                    "Automatic detection requires >= 2 episodes; "
                    "only %d file(s) found.  "
                    "For a single file use --import-timestamps.",
                    len(videos))
                sys.exit(1)
            else:
                bs = args.batch_size
            ...
            # Surface silent-audio warnings before the "no segments"
            # error so the user knows why detection failed.
            with _silent_lock:
                silent_names = list(_silent_episode_warnings)
            if silent_names:
                logger.warning(
                    "The following episode(s) were skipped because their "
                    "extracted audio was below the MIN_AUDIO_RMS=%.2e "
                    "threshold — fingerprinting was not attempted:\n  %s\n"
                    "If this show has very quiet audio, consider lowering "
                    "--min-audio-rms or checking that the correct audio "
                    "track is selected in the video file.",
                    CONSTANTS.MIN_AUDIO_RMS,
                    "\n  ".join(silent_names))

            if not results:
                logger.error(
                    "No segments detected.  Suggestions: try a different "
                    "--show-type, lower --similarity-threshold, widen "
                    "--intro-search-end, or check the log for near-silent "
                    "audio warnings.")
                sys.exit(1)
            ...
            if silent_names:
                print(f"Silent/skipped: {len(silent_names)} episode(s) "
